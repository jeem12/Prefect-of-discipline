...
$totalCount = $conn->query("SELECT * FROM `regist_stud_list` ORDER BY `id` DESC ")->num_rows;
$search_where = "";
if (!empty($search)) {
    $search_where = " where ";
...
$query = $conn->query("SELECT * FROM `regist_stud_list` {$search_where} ORDER BY {$columns_arr[$order[0]['column']]} {$order[0]['dir']} limit {$length} offset {$start} ");
$recordsFilterCount = $conn->query("SELECT * FROM `regist_stud_list` {$search_where} ")->num_rows;
