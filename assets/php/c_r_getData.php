...
$totalCount = $conn->query("SELECT * FROM `podms_claimed_items` ")->num_rows;
$search_where = "";
if(!empty($search)){
    $search_where = " where ";
...
$query = $conn->query("SELECT * FROM `podms_claimed_items` {$search_where} ORDER BY {$columns_arr[$order[0]['column']]} {$order[0]['dir']} limit {$length} offset {$start} ");
$recordsFilterCount = $conn->query("SELECT * FROM `podms_claimed_items` {$search_where} ")->num_rows;
...
while($row = $query->fetch_assoc()){
    $row['fullname'] = strtoupper($row['last_name'] . ', ' . $row['first_name']);
    $data[] = $row;
}
